...
static void	handle_infile(t_pipex *pipex, char **argv)
{
	if (pipex->here_doc)
...
static void	handle_outfile(t_pipex *pipex, char **argv, int argc)
{
	if (pipex->here_doc)
...
static void	handle_commands(t_pipex *pipex, char **argv,
		char **envp, int start)
{
	int	i;
...
void	parse_args(t_pipex *pipex, int argc, char **argv, char **envp)
{
	...
	if (pipex->here_doc)
		...
		start = 2;
	handle_infile(pipex, argv);
	handle_outfile(pipex, argv, argc);
	handle_commands(pipex, argv, envp, start);
}
